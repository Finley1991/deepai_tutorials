...
class Detector:
    ...
    def __pnet_detect(self, img):
        ...
        while min_side_len >= 12:
            ...
            #NCHW→HW，取每张图的第一个值C的高宽。因为输入的图像大于12*12，所以特征图是大于1*1的，因此得到的特征点需要H*W来定位的，
            # 而每个H,W对于的特征点反算回原图都有对于每个区域的置信度，所以HW是置信度的集合
            # NCHW→CHW，计算每张图上的坐标偏移率，也就是输出的4个通道上（x1,y1,x2,y2）偏移率的集合，每个偏移量也是HW来定位，总共有4个HW
